...
const CREDIT_PACKAGES: Record<string, { credits: number; amountCents: number }> = {
  single: { credits: 1, amountCents: 100 },
  triple: { credits: 3, amountCents: 500 },
};
...
Deno.serve(async (req) => {
  try {
    ...
    appendForm(params, "line_items[0][price_data][product_data][name]", `${credits} Nailed credits`);
    appendForm(params, "line_items[0][price_data][product_data][description]", `${credits} nail preview credit${credits === 1 ? "" : "s"}`);
